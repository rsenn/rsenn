/**
 * Minimal CommonMark/GFM subset renderer, sized to what the rsenn projects'
 * own markdown actually uses: ATX headings, fenced code, GFM tables, nested
 ...
 * as the first column of a GFM table (headed "Method", "Property", "Export",
 * ...), so tables whose header matches RE_MEMBER_TABLE get a per-row id +
 * anchor on that column, and render() returns those as `members` alongside
 * `headings` - one linkable, searchable anchor per documented member.
 *
 * Shared by every site built with tools/site/build.js. Runs under qjsm and
